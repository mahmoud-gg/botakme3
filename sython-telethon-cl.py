...
#logo
logo = pyfiglet.figlet_format('*      M𝐀𝐇𝐌𝐎𝐔𝐃 ࿐      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'sython':
        await bot.send_message(OWNER_ID, f"""**∘ بوت تجميع

∘ مطور ومبرمج البوت  - @VL_VD
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ M𝐀𝐇𝐌𝐎𝐔𝐃 ࿐ 𝐁𝐎𝐓 ༻', 'button0')]
        ]
        await bot.send_message(chat, '''M𝐀𝐇𝐌𝐎𝐔𝐃 ࿐ ⇲''', buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=1095638828))
